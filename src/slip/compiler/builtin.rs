...
impl<'ctx> Compiler<'ctx> {
    ...
    pub fn car(&mut self, expr: &Expression) -> Result<BasicValueEnum<'ctx>, &'static str> {
        ...
        match self.build_car(&result) {
            Ok(res) => Ok(res),
            Err(e) => Err(e),
        }
    }
    fn build_car(&self, arg: &BasicValueEnum<'ctx>) -> Result<BasicValueEnum<'ctx>, &'static str> {
        let func;
        ...
        let car_node = self.builder.build_load(car_ptr, "car_load");
        let car_value;
        match self.build_extract_struct_from_list_node(&car_node) {
            Some(value) => car_value = value,
        ...
        // Switch end
        self.builder.position_at_end(switch_end_block);
        let car_phi = self.builder.build_phi(self.struct_type, "car_phi");
        car_phi.add_incoming(&[
            (&self.nil_value, basic_blocks[SlipType::Nil as usize]),
            (&self.const_named_struct(SlipType::Error, 0.0, None, None, Some("Error: can't car t")), basic_blocks[SlipType::True as usize]),
            (&self.const_named_struct(SlipType::Error, 0.0, None, None, Some("Error: can't car Number")), basic_blocks[SlipType::Number as usize]),
            (&self.const_named_struct(SlipType::Error, 0.0, None, None, Some("Error: can't car String")), basic_blocks[SlipType::String as usize]),
            (&car_value, basic_blocks[SlipType::List as usize]),
            (&self.const_named_struct(SlipType::Error, 0.0, None, None, Some("Error: can't car Error")), basic_blocks[SlipType::Error as usize]),
            (&self.const_named_struct(SlipType::Error, 0.0, None, None, Some("Error: no matched type")), switch_else_block),
        ]);
        Ok(car_phi.as_basic_value())
    }
    ...
    pub fn add(&mut self, expr: &Expression) -> Result<BasicValueEnum<'ctx>, &'static str> {
        ...
        for arg in args {
            ...
            match self.build_struct_add(&accumulator, &result) {
                Ok(res) => accumulator = res,
                Err(e) => return Err(e),
            }
        }
        Ok(accumulator.into())
    }
    fn build_struct_add(&self, arg1: &BasicValueEnum<'ctx>, arg2: &BasicValueEnum<'ctx>) -> Result<BasicValueEnum<'ctx>, &'static str> {
        let func;
        match self.builder.get_insert_block() {
            Some(basic_block) => {
                match basic_block.get_parent() {
        ...
        let sub_phi = self.builder.build_phi(self.struct_type, "sub_phi");
        sub_phi.add_incoming(&[
            (&self.const_named_struct(SlipType::Error, 0.0, None, None, Some("Error: can't add nil")), basic_blocks[SlipType::Nil as usize]),
            (&self.const_named_struct(SlipType::Error, 0.0, None, None, Some("Error: can't add t")), basic_blocks[SlipType::True as usize]),
            (&add_res_struct, basic_blocks[SlipType::Number as usize]),
            (&self.const_named_struct(SlipType::Error, 0.0, None, None, Some("Error: can't add string")), basic_blocks[SlipType::String as usize]),
            (&self.const_named_struct(SlipType::Error, 0.0, None, None, Some("Error: can't add list")), basic_blocks[SlipType::List as usize]),
            (&self.const_named_struct(SlipType::Error, 0.0, None, None, Some("Error: can't add error")), basic_blocks[SlipType::Error as usize]),
            (&self.const_named_struct(SlipType::Error, 0.0, None, None, Some("Error: no matched type")), switch_else_block),
            (&self.const_named_struct(SlipType::Error, 0.0, None, None, Some("Error: type not matched")), else_bb),
        ]);
        Ok(sub_phi.as_basic_value())
    }
    pub fn sub(&mut self, expr: &Expression) -> Result<BasicValueEnum<'ctx>, &'static str> {
        let args = &expr.list.as_ref().unwrap().expressions[1..];
        if args.len() < 2 {
            return Err("sub expression takes >2 arguments")
        }
        let mut accumulator;
        match self.walk(&args[0]) {
            Ok(res) => accumulator = res,
        ...
        for arg in &args[1..] {
            ...
            match self.build_struct_sub(&accumulator, &result) {
                Ok(res) => accumulator = res,
                Err(e) => return Err(e),
            }
        }
    ...
    fn build_struct_sub(&self, arg1: &BasicValueEnum<'ctx>, arg2: &BasicValueEnum<'ctx>) -> Result<BasicValueEnum<'ctx>, &'static str> {
        let func;
        match self.builder.get_insert_block() {
            Some(basic_block) => {
                match basic_block.get_parent() {
            ...
            Some(arg1_number) => {
                ...
                match self.build_extract_value_from_struct(&arg2, StructIndex::Number) {
                    Some(arg2_number) => {
                        ...
                        let sub = self.builder.build_float_sub(arg1_number.into_float_value(), arg2_number.into_float_value(), "sub");
                        sub_res_struct = self.variable_to_struct(SlipType::Number, sub.into());
                        self.builder.build_unconditional_branch(end_bb);
                    },
                    None => return Err("Struct Number element not found"),
                }
        ...
        let sub_phi = self.builder.build_phi(self.struct_type, "sub_phi");
        sub_phi.add_incoming(&[
            (&self.const_named_struct(SlipType::Error, 0.0, None, None, Some("Error: can't sub nil")), basic_blocks[SlipType::Nil as usize]),
            (&self.const_named_struct(SlipType::Error, 0.0, None, None, Some("Error: can't sub t")), basic_blocks[SlipType::True as usize]),
            (&sub_res_struct, basic_blocks[SlipType::Number as usize]),
            (&self.const_named_struct(SlipType::Error, 0.0, None, None, Some("Error: can't sub string")), basic_blocks[SlipType::String as usize]),
            (&self.const_named_struct(SlipType::Error, 0.0, None, None, Some("Error: can't sub list")), basic_blocks[SlipType::List as usize]),
            (&self.const_named_struct(SlipType::Error, 0.0, None, None, Some("Error: can't sub error")), basic_blocks[SlipType::Error as usize]),
            (&self.const_named_struct(SlipType::Error, 0.0, None, None, Some("Error: no matched type")), switch_else_block),
            (&self.const_named_struct(SlipType::Error, 0.0, None, None, Some("Error: type not matched")), else_bb),
        ]);
        Ok(sub_phi.as_basic_value())
